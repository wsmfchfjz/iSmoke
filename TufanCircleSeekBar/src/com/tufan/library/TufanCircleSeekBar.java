...
public class TufanCircleSeekBar extends View {
    ...
    private void init(AttributeSet attrs, int defStyle) {
        ...
        format_to_time = a.getBoolean(R.styleable.TufanCircleSeekBar_format_to_time, false);
        ...
        int colorCode = a.getColor(R.styleable.TufanCircleSeekBar_wheel_active_color, Color.CYAN);

        mWheelActivePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mWheelActivePaint.setStyle(Style.STROKE);
        mWheelActivePaint.setStrokeWidth(mWheelStrokeWidth);
        mWheelActivePaint.setColor(colorCode);

        colorCode = a.getColor(R.styleable.TufanCircleSeekBar_wheel_unactive_color, Color.WHITE);

        mWheelUnactivePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mWheelUnactivePaint.setStyle(Style.STROKE);
        mWheelUnactivePaint.setStrokeWidth(mWheelStrokeWidth);
        mWheelUnactivePaint.setColor(colorCode);

        colorCode = a.getColor(R.styleable.TufanCircleSeekBar_wheel_second_active_color, Color.MAGENTA);

        mWheelSecondActivePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mWheelSecondActivePaint.setStyle(Style.STROKE);
        mWheelSecondActivePaint.setStrokeWidth(mWheelStrokeWidth);
        mWheelSecondActivePaint.setColor(colorCode);

        colorCode = a.getColor(R.styleable.TufanCircleSeekBar_pointer_color, Color.BLUE);

        mPointerPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mPointerPaint.setStrokeWidth(mPointerRadius);
        mPointerPaint.setColor(colorCode);

        colorCode = a.getColor(R.styleable.TufanCircleSeekBar_pointer_halo_color, Color.DKGRAY);

        mPointerHaloPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mPointerHaloPaint.setStrokeWidth(mPointerRadius);
        mPointerHaloPaint.setColor(colorCode);

        colorCode = a.getColor(R.styleable.TufanCircleSeekBar_text_color, Color.RED);

        mTextPaint = new Paint(Paint.ANTI_ALIAS_FLAG | Paint.LINEAR_TEXT_FLAG);
        mTextPaint.setStyle(Style.FILL_AND_STROKE);
        mTextPaint.setTextAlign(Align.LEFT);
        mTextPaint.setTextSize(mTextSize);
        mTextPaint.setColor(colorCode);
    ...
    public void setWheelActiveColorRes(int res) {
        mWheelActivePaint.setColor(getContext().getResources().getColor(res));
    }

    public void setWheelUnactiveColorRes(int res) {
        mWheelUnactivePaint.setColor(getContext().getResources().getColor(res));
    }

    public void setPointerColorRes(int res) {
        mPointerPaint.setColor(getContext().getResources().getColor(res));
    }
    ...
    public void setMaxProgress(int mMax) {
        this.mMax = mMax;
    }
    ...
    public void setStartProgress(int mStartProgress) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mShowText) {
            String textProgress = Integer.toString(mProgress + mStartProgress);
            if (format_to_time) {
                int timeMin = mProgress + mStartProgress;
                ...
                textProgress = hour + ":" + min;
            }
            mTextPaint.getTextBounds(textProgress, 0, textProgress.length(), mBounds);
            canvas.drawText(textProgress, (mWheelRectangle.centerX()) - (mTextPaint.measureText(textProgress) / 2), mWheelRectangle.centerY() + mBounds.height() / 2, mTextPaint);
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                float[] pointerPosition = calculatePointerPosition(mAngle);
                if (x >= (pointerPosition[0] - mPointerHaloRadius) && x <= (pointerPosition[0] + mPointerHaloRadius)
                        && y >= (pointerPosition[1] - mPointerHaloRadius) && y <= (pointerPosition[1] + mPointerHaloRadius)) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mUserIsMovingPointer) {
                    ...
                    Log.e("ACTION_MOVE", "old mAngle:" + mAngleOld);
                    ...
                    Log.d("ACTION_MOVE", "mAngleStart:" + mAngleStart + " mAngleEnd:" + mAngleEnd + " mAngle:" + mAngle + " mSweepAngle:" + mSweepAngle);

                    if (mAngle < mAngleStart - 10) {
                        Log.d("ACTION_MOVE", "mAngleStart:" + mAngleStart);
                        setProgress(0);
                        mUserIsMovingPointer = false;
                        return true;
                    }

                    if (mAngle > mAngleEnd + 10) {
                        Log.d("ACTION_MOVE", "mAngleEnd:" + mAngleEnd);
                        setProgress(mMax);
                        mUserIsMovingPointer = false;
                        return true;
                    }
    ...
    public int getProgress() {
        return mProgress + mStartProgress;
    }
